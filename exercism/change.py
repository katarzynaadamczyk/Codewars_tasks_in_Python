...
def find_fewest_coins(coins, target):
    ...
    calc_table = [[(target + 1, [], target) for _ in range(target + 1)]]
    for y, coin in enumerate(coins):
        new_line = [(target + 1, [], target)]
        for i in range(1, target + 1):
            if i >= coin:
                min_coins_so_far = deepcopy(min([new_line[i - coin]] + [calc_table[j][i - coin] for j in range(1, y + 1)], key=lambda x: (-x[2], len(x[1]), )))
                if calc_table[y][i][0] >= 1 + len(min_coins_so_far[1]) and min_coins_so_far[2] >= coin:
                    min_coins_so_far[1].append(coin)
                    new_line.append((len(min_coins_so_far[1]), min_coins_so_far[1], min_coins_so_far[2] - coin))
                else:
    ...
    for line in calc_table:
        print(line)
    if calc_table[len(coins)][target][2] > 0:
        raise ValueError("can't make target with given coins")
